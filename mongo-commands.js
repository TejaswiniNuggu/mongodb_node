...
//only time we can mix inclusion and exclusion is when u dont want the _id
db.movies.find({}, {_id:0,name:1,rating:1}).pretty();

//sorting -asc
db.movies.find({},{_id:0,name:1,rating:1}).sort({rating:1});
//if ratings are equal ,by name we set in asc
db.movies.find({},{_id:0,name:1,rating:1}).sort({rating:1,name:1});
//sorting -desc
db.movies.find({},{_id:0,name:1,rating:1}).sort({rating:-1});
//sorting with limit
db.movies.find({},{_id:0,name:1,rating:1}).sort({rating:-1}).limit(3);
//ofset in sql is skip here
db.movies.find({},{_id:0,name:1,rating:1}).sort({rating:-1}).limit(3).skip(1);
//updtae the data by including language to the data
db.movies.updateMany({},{$set:{language:"english"}});
db.movies.find({}, {_id:0,name:1,language:1})
//change language for the two movies to telugu
db.movies.find({name:{$in:["RRR","Baahubali"]}}).pretty();
db.movies.updateMany({name:{$in:["RRR","Baahubali"]}},
{$set:{language:"telugu"}}
);
//change language to tamil
db.movies.updateOne({name:"Jai Bhim"},
{$set:{language:"Tamil"}}
);
